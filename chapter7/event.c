...
#elif defined (__x86_64__)
#define SET_BIT(bit,field) __asm__ __volatile__ ("lock btsq %q1,%0":"=m"(field):"Ir"(bit):"memory" );
#define CLEAR_BIT(field, bit) __asm__ __volatile__ ("lock btrq %q1,%0":"=m" ((field)):"Ir"((bit)):"memory")
#else
#error "Unsupported architecture"
#endif
...
static inline unsigned long int xchg(volatile unsigned long int * old, unsigned long int value)
{
#if defined (__i386__)
	__asm__("xchgl %0,%1"
		:"=r" (value)
...
void do_hypervisor_callback(struct pt_regs *regs)
{
	...
	volatile vcpu_info_t *vcpu = &shared_info.vcpu_info[0];
